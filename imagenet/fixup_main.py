...
parser.add_argument('--alpha', default=0.7, type=float, help='interpolation strength')
...
def main_worker(gpu, ngpus_per_node, args_):
    ...
    parameters_weight = [p[1] for p in model.named_parameters() if ('conv' in p[0] or 'fc' in p[0]) and 'bias' not in p[0]]
    parameters_others = [p[1] for p in model.named_parameters() if not ('bias' in p[0] or 'scale' in p[0] or 'conv' in p[0] or 'fc' in p[0])]

    optimizer = torch.optim.SGD(
        [{'params': parameters_bias, 'lr': args_.lr/10., 'weight_decay': 0},
         {'params': parameters_scale, 'lr': args_.lr/10., 'weight_decay': 0},
         {'params': parameters_weight, 'weight_decay': args_.weight_decay},
    ...
    for epoch in pbar:
        ...
        acc1, acc5 = validate(val_loader, model, cel, args_)
...
def train(train_loader, model, criterion, optimizer, args_):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        images, target_a, target_b, lam = mixup_data(images, target, args_.alpha, use_cuda=True)
