"""
Original implementation of Fixup initialization from
        ...
        for m in self.modules():
            if isinstance(m, FixupBasicBlock):
                ...
                if m.downsample is not None:
                    nn.init.normal_(m.downsample.weight, mean=0,
                                    std=np.sqrt(
                ...
                nn.init.normal_(
                    m.conv2.weight, mean=0,
                    std=np.sqrt(
                        2 / (m.conv2.weight.shape[0] * np.prod(m.conv2.weight.shape[2:]))) * self.num_layers ** (-0.25))
                nn.init.constant_(m.conv3.weight, 0)
                if m.downsample is not None:
                    nn.init.normal_(
                        m.downsample.weight, mean=0,
                        std=np.sqrt(2 / (m.downsample.weight.shape[0] * np.prod(m.downsample.weight.shape[2:]))))
